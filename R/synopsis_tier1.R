...
arrow <- function(dir) {
  switch(dir, "rising" = "\u2191", "falling" = "\u2193", "flat" = "\u2192", "\u2014")
}
...
build_synopsis <- function(fred_data, kpis) {
  ...
  unemp   <- get_vals("UNRATE");    payrolls <- get_vals("PAYEMS")
  cpi     <- get_vals("CPIAUCSL");  fed      <- get_vals("FEDFUNDS")
  t10     <- get_vals("DGS10");     t2       <- get_vals("DGS2")
  mort    <- get_vals("MORTGAGE30US"); houst <- get_vals("HOUST")
  retail  <- get_vals("RSAFS");     vix      <- get_vals("VIXCLS")
  ...
  blocks[["labor"]] <- list(
    title = "Labor Market", color = "#00b4d8", icon = "users",
    stats = list(
    ...
    note = {
      ...
      if (!is.na(avg_payrolls_3m))
        np <- c(np, sprintf("3-month average payroll growth of %+.0fK/mo suggests a labor market that is %s.",
                            avg_payrolls_3m,
      ...
      if (!is.null(unemp) && !is.null(payrolls)) {
        ...
        if (!is.na(rc))
          np <- c(np, sprintf("36-month rolling correlation between unemployment level and payroll growth: %s — historically inverse as expected.", fmt_cor(rc)))
      }
      paste(np, collapse = " ")
    }
  )
  ...
  cpi_trend <- if (!is.null(cpi)) {
    yoy_series <- (cpi / lag(cpi, 12) - 1) * 100; trend_dir(yoy_series[!is.na(yoy_series)], 6)
  } else "unclear"
  cpi_peak <- if (!is.null(cpi) && length(cpi) >= 36) {
    ry <- zoo::rollapply(cpi, 13, function(x) (x[13]/x[1]-1)*100, fill=NA, align="right")
    ry <- ry[!is.na(ry)]
  ...
  blocks[["inflation"]] <- list(
    title = "Inflation", color = "#e94560", icon = "fire",
    stats = list(
    ...
    note = {
      ...
      if (!is.na(cpi_yoy_now) && !is.na(core_yoy_now)) {
        ...
        np <- c(np, sprintf("Headline CPI runs %s pp %s core, indicating food & energy %s are %s the overall print.",
                            abs(gap), if (gap>0) "above" else "below",
                            if (gap>0) "pressures" else "relief", if (gap>0) "elevating" else "tempering"))
      }
      if (!is.null(fed) && !is.na(cpi_yoy_now)) {
        rr <- round(tail(fed,1) - cpi_yoy_now, 1)
        np <- c(np, sprintf("Real Fed Funds rate (nominal \u2212 CPI): %.1f%% \u2014 %s.", rr,
                            if (rr>1.5) "restrictive territory, historically associated with demand cooling"
                            else if (rr>0) "mildly positive but below historical neutral"
  ...
  # ── 3. Rates & Yield Curve ───────────────────────────────────────────────────
  spread_now <- kpis$t10y2y %||% NA; t10_now <- kpis$t10yr %||% NA; mort_now <- kpis$mortgage30 %||% NA
  months_inverted <- if (!is.null(t10) && !is.null(t2)) {
    n <- min(length(t10), length(t2), 24); sum((tail(t10,n) - tail(t2,n)) < 0, na.rm=TRUE)
  } else NA
  mort_houst_lag <- if (!is.null(mort) && !is.null(houst)) {
    n <- min(length(mort), length(houst)); best_lag(tail(mort,n), tail(houst,n), max_lag=12)
  } else NULL
  mort_trend <- if (!is.null(mort)) trend_dir(mort, 12) else "unclear"

  blocks[["rates"]] <- list(
    title = "Rates & Yield Curve", color = "#f4a261", icon = "percent",
    stats = list(
      list(label = "10Y-2Y spread",
           value = if (!is.na(spread_now)) sprintf("%.2f pp (%s)", spread_now, if (spread_now<0) "INVERTED" else "normal") else "N/A",
      ...
      list(label = "Months inverted (last 24M)",
           ...
           col = if (!is.na(months_inverted) && months_inverted>12) "#e94560" else if (!is.na(months_inverted) && months_inverted>6) "#f4a261" else "#2dce89"),
      ...
      list(label = "Mortgage\u2013Housing starts lag",
           value = if (!is.null(mort_houst_lag)) sprintf("%d-mo lag, %s", mort_houst_lag$lag, fmt_cor(mort_houst_lag$r)) else "N/A",
           col = "#9aa3b2")
    ),
    note = {
      np <- c()
      if (!is.na(months_inverted) && months_inverted > 0)
        np <- c(np, sprintf("Yield curve has been inverted for %d of the past 24 months \u2014 a historically reliable (if lagged) recession signal, though timing varies widely.", months_inverted))
      if (!is.null(mort_houst_lag) && !is.na(mort_houst_lag$r))
        np <- c(np, sprintf("Mortgage rate leads housing starts by ~%d months (lag-adjusted %s, R\u00B2 \u2248 %.0f%%). Rate moves today are a forward indicator for construction activity.",
                            mort_houst_lag$lag, fmt_cor(mort_houst_lag$r), mort_houst_lag$r^2*100))
      paste(np, collapse = " ")
    }
  )

  # ── 4. Markets & Risk ────────────────────────────────────────────────────────
  vix_now <- kpis$vix %||% NA; oil_now <- kpis$oil_price %||% NA
  vix_trend  <- if (!is.null(vix)) trend_dir(vix, 12) else "unclear"
  vix_6m_avg <- if (!is.null(vix) && length(vix) >= 6)  round(mean(tail(vix,6),  na.rm=TRUE),1) else NA
  vix_1y_avg <- if (!is.null(vix) && length(vix) >= 12) round(mean(tail(vix,12), na.rm=TRUE),1) else NA
  oil_trend  <- if (!is.null(oil)) trend_dir(oil, 12) else "unclear"
  oil_6m_ago <- if (!is.null(oil) && length(oil) >= 130) oil[length(oil)-130] else NA
  oil_chg_pct <- if (!is.na(oil_6m_ago) && oil_6m_ago > 0) round((oil_now/oil_6m_ago-1)*100,1) else NA
  vix_cpi_cor <- if (!is.null(vix) && !is.null(cpi)) {
    n <- min(length(vix), length(cpi)); yoy <- (tail(cpi,n)/lag(tail(cpi,n),12)-1)*100
    roll_cor(tail(vix,n), yoy, 36)
  } else NA

  blocks[["markets"]] <- list(
    title = "Markets & Risk", color = "#7c5cbf", icon = "chart-bar",
    stats = list(
      list(label = "VIX regime",
           value = sprintf("%.1f (%s)", vix_now,
                           if (!is.na(vix_now) && vix_now>30) "elevated fear" else if (!is.na(vix_now) && vix_now>20) "cautious" else "complacent"),
           col = if (!is.na(vix_now) && vix_now>25) "#e94560" else if (!is.na(vix_now) && vix_now>18) "#f4a261" else "#2dce89"),
      list(label = "VIX: 6M avg vs 1Y avg",
           value = if (!is.na(vix_6m_avg) && !is.na(vix_1y_avg)) sprintf("%.1f vs %.1f (%s)", vix_6m_avg, vix_1y_avg, if (vix_6m_avg>vix_1y_avg) "stress rising" else "stress falling") else "N/A",
           col = if (!is.na(vix_6m_avg) && !is.na(vix_1y_avg) && vix_6m_avg>vix_1y_avg) "#e94560" else "#2dce89"),
    ...
    note = {
      np <- c()
      if (!is.na(vix_now))
        np <- c(np, sprintf("VIX at %.1f places equity vol in %s \u2014 %s.", vix_now,
                            ...
                            if (vix_now>30) "historically associated with risk-off positioning and tighter credit conditions" else if (vix_now>20) "watch for vol regime shift" else "market is pricing low near-term risk; watch for mean-reversion"))
      if (!is.na(oil_chg_pct))
        np <- c(np, sprintf("WTI crude %s%.0f%% over the past 6 months \u2014 a %s for headline CPI via energy component.",
                            if (oil_chg_pct>0) "+" else "", oil_chg_pct, if (abs(oil_chg_pct)>10) "meaningful impulse" else "modest move"))
      paste(np, collapse = " ")
    }
  )
...
render_synopsis_html <- function(blocks) {
  ...
  render_block <- function(b) {
    stats_html <- paste(vapply(b$stats, function(s) {
      sprintf("<div style='display:flex;justify-content:space-between;align-items:baseline;
         ...
         <span style='font-size:13px;font-weight:600;color:%s;'>%s</span>
              ...
              htmltools::htmlEscape(s$label), s$col, htmltools::htmlEscape(as.character(s$value)))
    ...
    note_html <- if (nchar(b$note) > 0) sprintf(
     ...
     </div>", b$color, htmltools::htmlEscape(b$note)) else ""

    sprintf("<div style='background:#1a2035;border:1px solid #2a3042;border-radius:8px;
       ...
       <div style='color:%s;font-size:11px;font-weight:700;text-transform:uppercase;
         ...
         <i class=\"fa fa-%s\" style=\"margin-right:6px;\"></i>%s
            ...
            b$color, b$color, b$icon, b$title, stats_html, note_html)
  }
  ...
  rows_html <- paste(vapply(seq(1, length(block_htmls), by=2), function(i) {
    left  <- block_htmls[i]
    right <- if (i+1 <= length(block_htmls)) block_htmls[i+1] else "<div style='flex:1;'></div>"
    sprintf("<div style='display:flex;gap:14px;margin-bottom:14px;'>
       <div style='flex:1;'>%s</div><div style='flex:1;'>%s</div></div>", left, right)
  }, character(1)), collapse="")

  ts <- format(Sys.time(), "%b %d, %Y %H:%M")
  HTML(paste0(
    sprintf("<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;'>
       ...
       </span><span style='color:#6b7585;font-size:11px;'>Computed %s</span></div>", ts),
    "<div style='padding:4px 2px;'>", rows_html,
...
.tab_synopsis_html <- function(title, color, icon_name, bullets, note = NULL) {
  bullet_html <- paste(vapply(bullets, function(b) {
    sprintf("<li style='margin-bottom:5px;'>
       <span style='color:%s;font-weight:600;'>%s</span>
       <span style='color:#d0d0d0;'>%s</span></li>",
            b$col %||% "#9aa3b2", htmltools::htmlEscape(b$label), htmltools::htmlEscape(b$text))
  }, character(1)), collapse="")
  ...
  note_html <- if (!is.null(note) && nchar(note) > 0) sprintf(
    ...
    color, color, htmltools::htmlEscape(note)) else ""
...
build_labor_synopsis    <- function(fred_data, kpis) { if (is.null(fred_data)||is.null(kpis)) return(NULL); htmltools::HTML(.tab_synopsis_html("Labor Market","#00b4d8","users", list(list(col="#00b4d8",label="Unemployment rate: ",text=sprintf("%.1f%%",kpis$unemp_rate%||%NA)),list(col="#2dce89",label="3M avg payrolls: ",text=sprintf("%+.0fK/mo",{p<-fred_data$PAYEMS; if(!is.null(p)&&nrow(p)>=4) round(mean(diff(tail(arrange(p,date)%>%pull(value),4)),na.rm=TRUE)) else NA}%||%NA)),list(col="#7c5cbf",label="Job openings: ",text=sprintf("%.0fK",kpis$job_openings%||%NA)),list(col="#f4a261",label="Wage growth YoY: ",text=sprintf("%.1f%%",kpis$wage_yoy%||%NA))))) }

build_inflation_synopsis <- function(fred_data, kpis) { if (is.null(fred_data)||is.null(kpis)) return(NULL); htmltools::HTML(.tab_synopsis_html("Inflation","#e94560","fire", list(list(col=if(!is.na(kpis$cpi_yoy%||%NA)&&(kpis$cpi_yoy%||%0)>3)"#e94560"else"#2dce89",label="CPI YoY: ",text=sprintf("%.1f%%",kpis$cpi_yoy%||%NA)),list(col="#f4a261",label="Core CPI YoY: ",text=sprintf("%.1f%%",kpis$core_cpi_yoy%||%NA)),list(col="#7c5cbf",label="Core PCE YoY: ",text=sprintf("%.1f%%",kpis$core_pce%||%NA)),list(col="#00b4d8",label="Real Fed Funds: ",text=sprintf("%.1f%%",(kpis$fed_funds%||%NA)-(kpis$cpi_yoy%||%NA)))))) }

build_housing_synopsis  <- function(fred_data, kpis) { if (is.null(fred_data)||is.null(kpis)) return(NULL); htmltools::HTML(.tab_synopsis_html("Housing","#2dce89","home", list(list(col="#2dce89",label="Housing starts: ",text=sprintf("%.0fK ann.",kpis$housing_starts%||%NA)),list(col="#f4a261",label="30yr mortgage: ",text=sprintf("%.2f%%",kpis$mortgage30%||%NA)),list(col="#00b4d8",label="Existing home sales: ",text=sprintf("%.2fM ann.",kpis$home_sales%||%NA))))) }

build_market_synopsis   <- function(fred_data, kpis) {
  if (is.null(fred_data)||is.null(kpis)) return(NULL)
  vix_now <- kpis$vix %||% NA; hy_now <- kpis$hy_spread %||% NA
  months_inv <- if (!is.null(fred_data$DGS10)&&!is.null(fred_data$DGS2)) {
    t10 <- arrange(fred_data$DGS10,date)$value; t2 <- arrange(fred_data$DGS2,date)$value
    n <- min(length(t10),length(t2),24); sum((tail(t10,n)-tail(t2,n))<0,na.rm=TRUE)
  } else NA
  hy_prev <- if (!is.null(fred_data$BAMLH0A0HYM2)&&nrow(fred_data$BAMLH0A0HYM2)>=2) {
    arrange(fred_data$BAMLH0A0HYM2,date)$value[nrow(fred_data$BAMLH0A0HYM2)-1]
  } else NA

  bullets <- list(
    list(col=if(!is.na(months_inv)&&months_inv>12)"#e94560"else"#2dce89",label="10Y-2Y spread: ",
         text=sprintf("inverted %d of last 24 months; spread %.2f pp",months_inv%||%0L,kpis$t10y2y%||%NA)),
    list(col=if(!is.na(vix_now)&&vix_now>25)"#e94560"else if(!is.na(vix_now)&&vix_now>18)"#f4a261"else"#2dce89",
         label="VIX: ",text=sprintf("%.1f \u2014 %s",vix_now,if(!is.na(vix_now)&&vix_now>30)"stress event"else if(!is.na(vix_now)&&vix_now>20)"cautious"else"risk-on")),
    list(col=if(!is.na(hy_now)&&hy_now>5)"#e94560"else"#2dce89",label="HY credit spread: ",
         text=sprintf("%.2f%% (prev %.2f%%)",hy_now,hy_prev%||%NA))
  )
  htmltools::HTML(.tab_synopsis_html("Financial Markets","#f4a261","chart-line",bullets,
    if(!is.na(months_inv)&&months_inv>0) sprintf("Curve inverted for %d of 24 months. Historically recession follows within 6-18 months.",months_inv) else NULL))
}
...
.score <- function(val, good_threshold, bad_threshold, higher_good = TRUE) {
  ...
  if (higher_good) { if (val >= good_threshold) return(1); if (val <= bad_threshold) return(-1); return(0) }
  else { if (val <= good_threshold) return(1); if (val >= bad_threshold) return(-1); return(0) }
}
...
build_glm_feature_matrix <- function(fred_data) {
  tryCatch({
    ...
    usrec <- fredr::fredr("USREC", observation_start = as.Date("1959-01-01"),
                          frequency = "m") %>%
      dplyr::select(date, recession = value) %>%
      dplyr::arrange(date)

    if (nrow(usrec) < 120) return(NULL)  # need at least 10 years
    ...
    if (is.null(unrate) || is.null(cpi_m) || is.null(fedfund)) return(NULL)
    ...
    if (!is.null(t10y2y)) {
      ...
      t10 <- gm("DGS10"); t2 <- gm("DGS2")
      if (is.null(t10) || is.null(t2)) return(NULL)
      spread_df <- dplyr::inner_join(t10 %>% dplyr::rename(t10=value),
                                     t2  %>% dplyr::rename(t2 =value), by="date") %>%
    ...
    pay_df <- if (!is.null(pay_m)) {
      pay_m %>% dplyr::arrange(date) %>%
        dplyr::mutate(pay_chg = value - dplyr::lag(value, 1),
    ...
    hy_df  <- if (!is.null(hy_m))   hy_m   %>% dplyr::rename(hy_spread  = value) else NULL
    ...
    lfpr_df<- if (!is.null(lfpr_m)) lfpr_m %>% dplyr::rename(prime_lfpr = value) else NULL
    ...
    isr_df <- if (!is.null(isr_m))  isr_m  %>% dplyr::rename(inv_sales = value) else NULL
    cc_df  <- if (!is.null(cc_m))   cc_m   %>% dplyr::rename(cc_del    = value) else NULL
    sent_df<- if (!is.null(sent_m)) sent_m %>% dplyr::rename(sentiment = value) else NULL

    # ── Create 12-month forward recession target ─────────────────────────────
    # rec_next12 = 1 if a recession month occurs in any of the next 12 months
    n <- nrow(usrec)
    usrec$rec_next12 <- vapply(seq_len(n), function(i) {
      idx <- (i + 1):min(i + 12, n)
    ...
    panel <- panel %>% dplyr::filter(!is.na(rec_next12), !is.na(yield_spread), !is.na(u_rise))
    if (nrow(panel) < 120) return(NULL)
...
train_recession_model <- function(fred_data) {
  ...
  possible_predictors <- c("yield_spread", "u_rise", "pay_3m", "real_rate",
                           "hy_spread", "oil_yoy", "prime_lfpr", "usd_yoy",
                           "inv_sales", "cc_del", "sentiment")
  ...
  if (nrow(panel_clean) < 100) return(NULL)
  ...
  tryCatch({
    ...
    message(sprintf("[synopsis_tier1] GLM trained on %d months, AIC=%.1f, %d predictors",
                    nrow(panel_clean), AIC(model), length(available)))
...
build_current_feature_vector <- function(yield_spread, u_rise, pay_3m, real_rate,
                                         hy_v, oil_yoy, prime_age_lfpr, usd_yoy,
                                         inventory_sales_ratio, cc_delinquency, cs) {
  data.frame(
...
.compute_recession_prob <- function(
  ...
  if (!is.null(glm_result) && !is.null(glm_result$model)) {
    ...
    feat_df <- build_current_feature_vector(
      yield_spread, u_rise, pay_3m, real_rate, hy_v, oil_yoy,
    ...
    if (!is.na(glm_prob)) {
      ...
      for (pred in keep_cols) {
        ...
        x   <- feat_vals[[pred]]
        z   <- if (pred %in% coef_tbl$predictor) coef_tbl$z_stat[coef_tbl$predictor == pred] else NA_real_
        lbl <- if (pred %in% names(pretty_labels)) pretty_labels[[pred]] else pred
      ...
      contrib_df <- do.call(rbind, lapply(names(contribs), function(id) {
        r <- contribs[[id]]
        data.frame(name=r$name, contribution=r$contribution, z_stat=r$z_stat, stringsAsFactors=FALSE)
      }))
      ...
      contrib_sorted <- contrib_df[order(contrib_df$contribution, decreasing=TRUE), ]
      drivers_up   <- head(contrib_sorted$name[contrib_sorted$contribution > 0.1], 4)
      drivers_down <- head(contrib_sorted$name[contrib_sorted$contribution < -0.1], 3)

      return(list(prob=prob, tier=tier, drivers_up=drivers_up, drivers_down=drivers_down,
                  factor_contributions=contrib_df,
                  model_type="GLM (estimated)", n_obs=glm_result$n_obs, aic=glm_result$aic))
    }
  ...
  if (!is.na(yield_spread)) {
    ...
    if (!is.na(inv_months)) {
      yc_delta <- yc_delta + if (inv_months>=18) 1.5 else if (inv_months>=12) 1.2 else if (inv_months>=6) 0.6 else if (inv_months>=3) 0.3 else 0
    }
  }
  ...
  if (!is.na(u) && !is.null(unemp) && length(unemp) >= 12) {
    ...
    sahm_delta <- sahm_delta + if (u_rise>=0.5) 1.8 else if (u_rise>=0.3) 0.9 else if (u_rise>=0.1) 0.3 else if (u_rise<(-0.2)) -0.4 else 0
  }
  if (!is.na(pay_3m)) {
    sahm_delta <- sahm_delta + if (pay_3m<(-50)) 1.5 else if (pay_3m<0) 0.6 else if (pay_3m<100) 0.1 else if (pay_3m>250) -0.5 else 0
  }
  ...
  if (!is.na(prime_age_lfpr)) {
    lfpr_delta <- lfpr_delta + if (prime_age_lfpr<80.5) 0.8 else if (prime_age_lfpr<82.0) 0.4 else if (prime_age_lfpr>83.5) -0.3 else 0
  }
  if (!is.na(prime_age_lfpr_chg)) {
    lfpr_delta <- lfpr_delta + if (prime_age_lfpr_chg<(-0.5)) 0.9 else if (prime_age_lfpr_chg<(-0.2)) 0.4 else if (prime_age_lfpr_chg>0.3) -0.3 else 0
  }
  ...
  if (!is.na(jobless_claims_trend)) {
    claims_delta <- if (jobless_claims_trend>0.20) 1.2 else if (jobless_claims_trend>0.10) 0.6 else if (jobless_claims_trend>0.05) 0.2 else if (jobless_claims_trend<(-0.10)) -0.3 else 0
  }
  ...
  quits_delta <- 0
  if (!is.na(quits_yoy)) {
    quits_delta <- if (quits_yoy<(-15)) 0.8 else if (quits_yoy<(-8)) 0.4 else if (quits_yoy<(-3)) 0.2 else if (quits_yoy>8) -0.3 else 0
  }
  ...
  oil_delta <- 0
  if (!is.na(oil_yoy)) {
    oil_delta <- if (oil_yoy>50) 1.0 else if (oil_yoy>25) 0.6 else if (oil_yoy>15) 0.2 else if (oil_yoy<(-30)) 0.5 else if (oil_yoy<(-15)) 0.2 else if (oil_yoy>0&&oil_yoy<10) -0.1 else 0
  }
  ...
  if (!is.na(equity_drawdown_pct)) {
    ...
    eq_delta <- if (dd>30) 1.6 else if (dd>20) 1.1 else if (dd>10) 0.5 else if (dd<5) -0.2 else 0
  }
  ...
  hy_delta <- 0
  if (!is.na(hy_v)) {
    hy_delta <- if (hy_v>7.0) 1.4 else if (hy_v>5.5) 0.7 else if (hy_v>4.5) 0.2 else if (hy_v<3.5) -0.4 else 0
  }
  ...
  usd_delta <- 0
  if (!is.na(usd_yoy)) {
    usd_delta <- if (usd_yoy>12) 0.8 else if (usd_yoy>8) 0.4 else if (usd_yoy>4) 0.1 else if (usd_yoy<(-5)) -0.2 else 0
  }
  ...
  rr_delta <- 0
  if (!is.na(real_rate)) {
    rr_delta <- if (real_rate>3.0) 1.0 else if (real_rate>2.0) 0.5 else if (real_rate>1.0) 0.2 else if (real_rate<0) -0.3 else 0
  }
  ...
  sent_delta <- 0
  if (!is.na(cs)) {
    sent_delta <- if (cs<60) 0.7 else if (cs<70) 0.3 else if (cs>90) -0.4 else 0
  }
  ...
  if (!is.na(inventory_sales_ratio)) {
    inv_delta <- if (inventory_sales_ratio>1.55) 0.7 else if (inventory_sales_ratio>1.45) 0.3 else if (inventory_sales_ratio>1.40) 0.1 else if (inventory_sales_ratio<1.30) -0.2 else 0
  }
  ...
  if (!is.na(cc_delinquency)) {
    cc_delta <- if (cc_delinquency>4.0) 0.9 else if (cc_delinquency>3.0) 0.5 else if (cc_delinquency>2.5) 0.2 else if (cc_delinquency<1.8) -0.3 else 0
  }
  ...
  # Driver lists
  drivers_up   <- character(0); drivers_down <- character(0)
  ...
  if (!is.null(unemp) && length(unemp) >= 12) {
    ...
    if (ur >= 0.3) drivers_up <- c(drivers_up, sprintf("Unemployment %.1f pp above 12M low",ur))
  }
  if (!is.na(equity_drawdown_pct) && abs(equity_drawdown_pct) > 10)
    drivers_up <- c(drivers_up, sprintf("Equity drawdown %.0f%% from 52-week high", equity_drawdown_pct))
  ...
  list(prob=prob, tier=tier,
       drivers_up=head(drivers_up,4), drivers_down=head(drivers_down,3),
       factor_contributions=contrib_df, model_type="Hand-tuned log-odds")
}


#' Shared tier classifier
.classify_recession_tier <- function(prob) {
  if (prob >= 60) list(label="Elevated",     color="#e94560", icon="exclamation-triangle",
                       desc="Multiple leading indicators aligned recessionary. Historical hit rate ~65%. Defensive positioning warranted.")
  else if (prob >= 40) list(label="Moderate",    color="#f4a261", icon="exclamation-circle",
  ...
  else if (prob >= 20) list(label="Low\u2013Moderate", color="#f4a261", icon="minus-circle",
...
.build_factor_bar_chart <- function(factor_contributions, model_type = "Hand-tuned log-odds") {
  ...
  fc <- factor_contributions[order(abs(factor_contributions$contribution), decreasing=TRUE), ]
  fc <- head(fc, 13)

  max_abs <- max(abs(fc$contribution), na.rm=TRUE)
  if (max_abs < 0.01) max_abs <- 1
  ...
  rows <- vapply(seq_len(nrow(fc)), function(i) {
    ...
    col   <- if (delta > 0.02) "#e94560" else if (delta < -0.02) "#2dce89" else "#9aa3b2"
    dir   <- if (delta >= 0) "left:calc(50% + 1px)" else sprintf("left:calc(50%% - %dpx)", as.integer(pct * 3))
    ...
    z_badge <- if (has_z && !is.na(row$z_stat)) {
      z_col <- if (abs(row$z_stat) > 2.5) "#d0d0d0" else if (abs(row$z_stat) > 1.5) "#9aa3b2" else "#555"
      sprintf("&nbsp;<span style='color:%s;font-size:9px;'>(z=%.1f)</span>", z_col, row$z_stat)
    } else ""
    ...
    sprintf(
      ...
      htmltools::htmlEscape(row$name), z_badge, bar_style, col, delta)
  }, character(1))
...
build_growth_outlook <- function(fred_data, kpis, mkt_returns = NULL) {
  ...
  mort    <- gv("MORTGAGE30US"); houst <- gv("HOUST"); retail <- gv("RSAFS")
  sent    <- gv("UMCSENT"); indpro   <- gv("INDPRO");  vix_vals <- gv("VIXCLS")
  hy      <- gv("BAMLH0A0HYM2"); oil <- gv("DCOILWTICO")
  ...
  hy_v   <- kpis$hy_spread %||% NA;  oil_v  <- kpis$oil_price %||% NA
  ...
  vix_6m_avg <- if (!is.null(vix_vals) && length(vix_vals) >= 126) mean(tail(vix_vals,126), na.rm=TRUE) else vix_v
  ...
  components <- list(
    labor      = list(weight=2.5, label="Labor Market",
                      ...
                      detail=if(!is.na(u)&&!is.na(pay_3m)) sprintf("%.1f%% unemployment; avg payrolls %+.0fK/mo (3M)",u,pay_3m) else "N/A"),
    consumer   = list(weight=2.0, label="Consumer Demand",
                      score=mean(c(.score(ret_real,1.0,-1.0,TRUE),.score(cs,85,65,TRUE)), na.rm=TRUE),
                      detail=if(!is.na(ret_real)&&!is.na(cs)) sprintf("Real retail YoY %.1f%%; sentiment %.0f",ret_real,cs) else "N/A"),
    monetary   = list(weight=2.0, label="Monetary Conditions",
                      ...
                      detail=if(!is.na(real_rate)&&!is.na(yield_spread)) sprintf("Real rate %.1f%%; 10Y-2Y %.2f pp; %d mo inverted",real_rate,yield_spread,inv_months) else "N/A"),
    housing    = list(weight=1.5, label="Housing",
                      score=mean(c(.score(hs,1400,1000,TRUE),.score(mort_v,6.0,7.5,FALSE)), na.rm=TRUE),
                      detail=if(!is.na(hs)&&!is.na(mort_v)) sprintf("%.0fK starts ann.; %.2f%% mortgage rate",hs,mort_v) else "N/A"),
    financial  = list(weight=1.5, label="Financial Conditions",
                      score=mean(c(.score(vix_v,18,28,FALSE),.score(hy_v,4.5,6.0,FALSE)), na.rm=TRUE),
                      detail=if(!is.na(vix_v)&&!is.na(hy_v)) sprintf("VIX %.1f; HY spread %.2f%%",vix_v,hy_v) else "N/A"),
    inflation  = list(weight=1.5, label="Inflation",
                      score=mean(c(.score(cpi_y,2.5,4.5,FALSE),.score(pce_y,2.3,3.5,FALSE)), na.rm=TRUE),
                      detail=if(!is.na(cpi_y)&&!is.na(pce_y)) sprintf("CPI YoY %.1f%%; Core PCE %.1f%%",cpi_y,pce_y) else "N/A"),
  ...
  regime <- dplyr::case_when(
    score_0_10 >= 7.5 ~ list(label="Expansion",         color="#2dce89", icon="arrow-up",
                              gdp_est="+2.5% to +3.5%",  summary="Broad-based growth signals."),
    score_0_10 >= 5.5 ~ list(label="Moderate Growth",   color="#00b4d8", icon="minus",
                              gdp_est="+1.0% to +2.5%",  summary="Below-trend but positive growth likely. Mixed signals."),
    score_0_10 >= 3.5 ~ list(label="Stall / Slowdown",  color="#f4a261", icon="arrow-down",
                              gdp_est="-0.5% to +1.0%",  summary="Growth at risk. Restrictive monetary conditions or financial stress beginning to bite."),
    TRUE              ~ list(label="Contraction Risk",  color="#e94560", icon="exclamation-triangle",
  ...
  swing_factors$biggest_drag    <- list(name=components[[drag]]$label, score=round(components[[drag]]$score,2), detail=components[[drag]]$detail)
  supp <- names(which.max(sapply(components, `[[`, "score")))
  swing_factors$biggest_support <- list(name=components[[supp]]$label, score=round(components[[supp]]$score,2), detail=components[[supp]]$detail)
  swing_factors$yield_curve_watch <- if (inv_months>6) sprintf("Yield curve inverted %d months.",inv_months) else if (!is.na(yield_spread)&&yield_spread>0) sprintf("Yield curve re-steepened to %.2f pp.",yield_spread) else NULL
  swing_factors$fed_watch <- if (!is.na(real_rate)&&real_rate>2.0) sprintf("Real Fed Funds %.1f%% — restrictive.",real_rate) else if (!is.na(real_rate)&&real_rate<0.5) "Real rate near zero." else NULL
  ...
  glm_result <- tryCatch({
    ...
    needs_train <- !exists(cache_name, envir=.GlobalEnv) ||
      ...
      difftime(Sys.time(), get(cache_name, envir=.GlobalEnv)$trained_at, units="days") > 30
  ...
  }, error = function(e) { message("[synopsis_tier1] GLM cache error: ", e$message); NULL })

  recession_prob <- .compute_recession_prob(
    yield_spread=yield_spread, inv_months=inv_months, unemp=unemp, u=u,
    pay_3m=pay_3m, real_rate=real_rate, hy_v=hy_v, vix_v=vix_v, cs=cs,
    ip_y=ip_y, hs=hs,
...
render_growth_outlook_html <- function(outlook) {
  if (is.null(outlook))
    return(HTML("<p style='color:#6b7585;padding:16px;'>Growth outlook not yet computed.</p>"))

  reg <- outlook$regime; score <- outlook$score; comps <- outlook$components; swing <- outlook$swing
  pct <- score/10*100; gauge_col <- reg$color
  ...
  gauge_html <- sprintf(
    ...
    if(score<5)"#e94560" else "#f4a261", gauge_col, pct, gauge_col, score)
  ...
  comp_html <- paste(vapply(names(comps), function(nm) {
    ...
    sprintf("<div style='margin-bottom:8px;'>
       ...
       <div style='color:#6b7585;font-size:10px;margin-top:2px;'>%s</div></div>",
            c$label, col, if(s>=0.4)"Positive" else if(s>=-0.4)"Neutral" else "Negative",
            col, bar_w, htmltools::htmlEscape(c$detail))
  }, character(1)), collapse="")
  ...
  swing_html <- paste(c(
    ...
    if(!is.null(swing$fed_watch))       sprintf("<li style='margin-bottom:6px;'><span style='color:#00b4d8;font-weight:600;'>\u2699 Fed Policy:</span> %s</li>", htmltools::htmlEscape(swing$fed_watch))
  ), collapse="")

  rp <- outlook$recession_prob; rp_html <- ""
  if (!is.null(rp)) {
    prob <- rp$prob; tier <- rp$tier; bar_col <- tier$color
    up_html   <- if(length(rp$drivers_up)>0)   paste(sprintf("<li style='margin-bottom:4px;'><span style='color:#e94560;'>\u25b2</span> %s</li>", htmltools::htmlEscape(rp$drivers_up)),   collapse="") else ""
    down_html <- if(length(rp$drivers_down)>0)  paste(sprintf("<li style='margin-bottom:4px;'><span style='color:#2dce89;'>\u25bc</span> %s</li>", htmltools::htmlEscape(rp$drivers_down)), collapse="") else ""
    drivers_section <- if(nchar(up_html)>0||nchar(down_html)>0)
      sprintf("<ul style='list-style:none;padding:0;margin:6px 0 0;font-size:11px;color:#d0d0d0;'>%s%s</ul>", up_html, down_html) else ""

    model_badge <- if (!is.null(rp$model_type) && rp$model_type == "GLM (estimated)") {
      sprintf("<span style='background:rgba(45,206,137,0.12);color:#2dce89;border:1px solid rgba(45,206,137,0.3);
               border-radius:8px;padding:1px 8px;font-size:9px;font-weight:600;margin-left:6px;'>
               GLM n=%d AIC=%.0f</span>", rp$n_obs %||% 0L, rp$aic %||% 0)
    } else "<span style='color:#9aa3b2;font-size:9px;margin-left:6px;'>hand-tuned</span>"
    ...
    rp_html <- sprintf(
      ...
      bar_col, tier$icon, bar_col, model_badge,
      bar_col, sprintf("%.0f",prob),
      bar_col, bar_col, bar_col, tier$label,
      sprintf("%.0f", min(prob,98)), bar_col,
      htmltools::htmlEscape(tier$desc), drivers_section)
  }

  HTML(paste0(
    sprintf("<div style='display:flex;gap:14px;'>
       ...
       <div style='flex:1;display:flex;flex-direction:column;gap:10px;'>
         ...
         <div style='background:#1a2035;border:1px solid #2a3042;border-radius:8px;padding:14px 16px;'>
           ...
           reg$color, reg$color, reg$icon, reg$color, reg$label, reg$gdp_est,
           gauge_html, reg$color, htmltools::htmlEscape(reg$summary),
           length(comps), format(outlook$as_of, "%b %d, %Y"), rp_html, comp_html, swing_html),
    .build_model_modal(factor_contributions = if(!is.null(outlook$recession_prob)) outlook$recession_prob$factor_contributions else NULL,
                       model_type = if(!is.null(outlook$recession_prob)) outlook$recession_prob$model_type %||% "Hand-tuned log-odds" else "Hand-tuned log-odds")
  ))
}
...
.build_model_modal <- function(factor_contributions = NULL, model_type = "Hand-tuned log-odds") {
  ...
  # ── Factor detail table ────────────────────────────────────────────────────
  factors <- list(
    list(rank=1,  name="Yield Curve (10Y-2Y spread)",    weight="High",   col="#e94560",
    ...
    list(rank=2,  name="Sahm Rule / Labor Momentum",     weight="High",   col="#e94560",
    ...
    list(rank=3,  name="Prime-Age LFPR (25\u201354)",        weight="Medium", col="#f4a261",
    ...
    list(rank=4,  name="Jobless Claims Trend",           weight="Medium", col="#f4a261",
    ...
    list(rank=5,  name="Quits Rate YoY Change",          weight="Medium", col="#f4a261",
    ...
    list(rank=6,  name="Oil Price Shock",                weight="Medium", col="#f4a261",
    ...
    list(rank=7,  name="Equity Bear Market (SPY drawdown)", weight="Medium", col="#f4a261",
    ...
    list(rank=8,  name="HY Credit Spreads",              weight="Medium", col="#f4a261",
    ...
    list(rank=9,  name="USD Surge (YoY)",                weight="Low",    col="#9aa3b2",
    ...
    list(rank=10, name="Real Fed Funds Rate",            weight="Low",    col="#9aa3b2",
    ...
    list(rank=11, name="Consumer Sentiment",             weight="Low",    col="#9aa3b2",
    ...
    list(rank=12, name="Inventory-to-Sales Ratio",       weight="Low",    col="#9aa3b2",
    ...
    list(rank=13, name="Credit Card Delinquency Rate",   weight="Low",    col="#9aa3b2",
  ...
  rows <- paste(vapply(factors, function(f) {
    wt_col <- switch(f$weight, "High"="#e94560", "Medium"="#f4a261", "#9aa3b2")
    sprintf(
      ...
      f$col, f$rank, htmltools::htmlEscape(f$name),
      wt_col, wt_col, wt_col, f$weight,
