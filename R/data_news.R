...
NEWS_QUERY <- paste(
  ...
  '"unemployment" OR "jobs report" OR "housing market" OR "earnings" OR "labor market',
  collapse = " "
)
...
fetch_news_raw <- function(days_back = 4, max_articles = 100) {
  ...
  from_date <- format(
    Sys.Date() - lubridate::days(days_back),
    "%Y-%m-%dT00:00:00"
  )
  ...
  tryCatch(
    ...
    error = function(e) {
      message("News fetch error: ", e$message)
      NULL
    }
  )
}
...
fetch_news <- function(days_back = 4, max_articles = 80, cache_days = 10) {
  ...
  all_history <- dplyr::bind_rows(cached, fresh) %>%
    ...
    dplyr::filter(
      as.numeric(difftime(Sys.time(), published, units = "hours")) <
        cache_days * 24
    ) %>%
    dplyr::mutate(
      ...
      days_in_cache = pmax(
        1L,
        as.integer(difftime(Sys.Date(), as.Date(published), units = "days")) +
          1L
      )
    ) %>%
    dplyr::arrange(dplyr::desc(relevance_score), dplyr::desc(published))
