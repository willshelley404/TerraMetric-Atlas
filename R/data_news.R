...
.news_cache_dir <- local({
  ...
  if (nchar(env_dir) > 0) {
    ...
    if (file.access(env_dir, 2) == 0) return(env_dir)
...
load_news_cache <- function() {
  if (is.null(NEWS_CACHE_FILE)) return(NULL)
  tryCatch({
...
save_news_cache <- function(df) {
  if (is.null(NEWS_CACHE_FILE)) return(invisible(NULL))
...
fetch_news <- function(days_back = 4, max_articles = 80, cache_days = 10) {
  ...
  if (is.null(all_rows) || nrow(all_rows) == 0) {
    if (!is.null(fresh) && nrow(fresh) > 0) return(structure(fresh, full_history=fresh))
  ...
  all_history <- all_rows %>%
    ...
    dplyr::mutate(
      days_in_cache = pmax(1L,
        as.integer(difftime(as.Date(now_utc), as.Date(published_utc), units="days")) + 1L)
    ) %>%
  ...
  if (nrow(display_view) == 0 && nrow(all_history) > 0) {
    message("[News] display_view empty after filter — showing most recent from cache")
