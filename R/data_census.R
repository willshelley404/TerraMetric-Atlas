...
fetch_state_acs <- function(year = NULL, survey = "acs5") {
  ...
  tryCatch({
    ...
    if (is.null(year)) year <- .best_acs_year(2023)

    message(sprintf("[Census] Fetching ACS %s for %d (no geometry)...", survey, year))
    ...
    states_sf <- tigris::states(cb = TRUE, resolution = "20m", year = year) %>%
  ...
  tryCatch({
    ...
    raw %>%
      ...
      filter(
        !is.na(unemp_rate),
        pop > 250000,
        # Exclude Puerto Rico and all US territories by NAME
        # CBSAs spanning PR have "PR" in their state abbreviation portion
        !grepl("\\bPR\\b|Puerto Rico|Guam|Virgin Islands|Samoa|Mariana",
...
VAR_CFG <- list(
  ...
  vacancy_rate = list(label="Housing Vacancy",   fmt="%",  pal="RdYlGn", rev=TRUE,
                      desc="Higher = worse (red)")
)
...
build_state_map <- function(state_sf, variable = "unemp_rate") {
  ...
  if (!variable %in% names(state_sf)) {
    message(sprintf("[Map] variable '%s' not in sf columns: %s",
                    variable, paste(names(state_sf), collapse=", ")))
    return(NULL)
  }
  ...
  message(sprintf("[Map] Building choropleth: %s, %d states, range [%.1f, %.1f]",
  ...
  popup_vec <- vapply(seq_len(nrow(state_sf)), function(i) {
    ...
    val_str <- if (is.na(v)) "N/A" else if (cfg$fmt == "$")
      paste0("$", format(round(v), big.mark = ","))
    else
      paste0(round(v, 1), "%")

    inc_str  <- if (!is.na(r$med_income))   paste0("$", format(round(as.numeric(r$med_income)),   big.mark=",")) else "N/A"
    home_str <- if (!is.na(r$med_home_val)) paste0("$", format(round(as.numeric(r$med_home_val)), big.mark=",")) else "N/A"
  ...
  leaflet::leaflet(state_sf,
    ...
    leaflet::addPolygons(
      ...
      labelOptions = leaflet::labelOptions(
        style     = list("background" = "#1e2640", "color" = "#e0e0e0",
...
fetch_metro_sf <- function(metro_df, year = 2023) {
  ...
  tryCatch({
    ...
    # Exclude territory CBSAs by NAME (GEOID prefix is unreliable for CBSAs)
    cbsa_centroids <- cbsa_sf %>%
      filter(!grepl("\\bPR\\b|Puerto Rico|Guam|Virgin Islands|Samoa|Mariana",
                    NAME, ignore.case = TRUE)) %>%
      sf::st_centroid() %>%
      mutate(
    ...
    message(sprintf("[Metro map] CBSA centroid fetch failed: %s", conditionMessage(e)))
    NULL
  })
}

#' Build metro-level bubble map
build_metro_bubble_map <- function(metro_sf_df, variable = "unemp_rate",
                                   min_pop = 500000) {
  ...
  df <- metro_sf_df %>%
    filter(
      ...
      !is.na(.data[[variable]]),
      !is.na(lon), !is.na(lat),
      # Belt-and-suspenders: exclude territory metros by NAME
      !grepl("\\bPR\\b|Puerto Rico|Guam|Virgin Islands|Samoa|Mariana",
             metro, ignore.case = TRUE)
    )
  ...
  popup_vec <- vapply(seq_len(nrow(df)), function(i) {
    ...
    paste0(
      ...
      "<b style='color:#00b4d8;font-size:14px;'>",
        htmltools::htmlEscape(as.character(r$metro)), "</b><br/>",
      "<table style='width:100%;font-size:12px;'>",
      "<tr><td style='color:#9aa3b2;'>", cfg$label, "</td>",
          "<td style='color:#e0e0e0;font-weight:600;'>", fmt_val(as.numeric(r[[variable]])), "</td></tr>",
      "<tr><td style='color:#9aa3b2;'>Population</td>",
          "<td>", format(as.integer(r$pop), big.mark=","), "</td></tr>",
      "<tr><td style='color:#9aa3b2;'>Unemp. Rate</td>",
          "<td>", round(as.numeric(r$unemp_rate),  1), "%</td></tr>",
