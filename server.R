...
server <- function(input, output, session) {
  ...
  w <- Waiter$new(
    html = tagList(
      spin_dots(),
      br(),
      div(style = "color:#e0e0e0;font-size:14px;margin-top:16px;", "Loading EconPulse AI…"),
      div(style = "color:#9aa3b2;font-size:12px;margin-top:6px;", "Fetching FRED · BLS · Markets · News")
    ),
  ...
  observeEvent(input$refresh_data, {
    ...
    load_all_data()
    w$hide()
    showNotification("Data refreshed successfully!", type = "message", duration = 4)
  })
  ...
  observeEvent(input$tabs, {
    if (input$tabs == "forecasting" && is.null(rv$forecasts) && !is.null(rv$fred_data)) {
    ...
    plot_ly() %>%
      layout(
        ...
        annotations = list(list(text = msg, x = 0.5, y = 0.5, xref = "paper", yref = "paper",
                                showarrow = FALSE, font = list(color = "#9aa3b2", size = 14)))
  ...
  pct_badge <- function(val, label = "", higher_good = TRUE) {
    if (is.na(val)) return("")
    cls <- if (val > 0) { if (higher_good) "positive" else "negative" }
           else if (val < 0) { if (higher_good) "negative" else "positive" }
           else "neutral"
    arrow <- if (val > 0) "▲" else if (val < 0) "▼" else "—"
    paste0('<span class="metric-change ', cls, '">', arrow, " ", label, round(abs(val), 2), "</span>")
  }

  output$kpi_unemp    <- renderText({ req(rv$kpis); paste0(round(rv$kpis$unemp_rate %||% NA, 1), "%") })
  output$kpi_unemp_chg <- renderUI({ req(rv$kpis); HTML(pct_badge(rv$kpis$unemp_rate %||% NA - 3.5, "vs 3.5%", FALSE)) })

  output$kpi_cpi      <- renderText({ req(rv$kpis); fmt_pct(rv$kpis$cpi_yoy) })
  output$kpi_cpi_chg  <- renderUI({ req(rv$kpis); HTML(pct_badge(rv$kpis$cpi_yoy %||% NA - 2, "vs 2% target", FALSE)) })

  output$kpi_fedfunds <- renderText({ req(rv$kpis); fmt_pct(rv$kpis$fed_funds) })
  output$kpi_fedfunds_chg <- renderUI({ HTML("") })

  output$kpi_payrolls <- renderText({ req(rv$kpis); fmt_num(rv$kpis$payrolls_chg, 0, "K") })
  ...
  output$kpi_oil    <- renderText({ req(rv$kpis); fmt_dollar(rv$kpis$oil_price) })
  output$kpi_oil_chg <- renderUI({ HTML("") })

  output$kpi_vix    <- renderText({ req(rv$kpis); fmt_num(rv$kpis$vix) })
  output$kpi_vix_chg <- renderUI({
    req(rv$kpis)
    v <- rv$kpis$vix %||% NA
    cls <- if (!is.na(v) && v > 20) "negative" else "positive"
    HTML(paste0('<span class="metric-change ', cls, '">', if (!is.na(v) && v > 20) "Elevated" else "Low", '</span>'))
  })

  output$kpi_t10yr   <- renderText({ req(rv$kpis); fmt_pct(rv$kpis$t10yr) })
  output$kpi_t10yr_chg <- renderUI({ HTML("") })

  output$kpi_t10y2y  <- renderText({ req(rv$kpis); fmt_pct(rv$kpis$t10y2y) })
  output$kpi_t10y2y_chg <- renderUI({
    req(rv$kpis)
    v <- rv$kpis$t10y2y %||% NA
    if (is.na(v)) return(HTML(""))
    cls <- if (v < 0) "negative" else "positive"
    HTML(paste0('<span class="metric-change ', cls, '">', if (v < 0) "⚠ Inverted" else "Normal", '</span>'))
  })

  output$kpi_mortgage <- renderText({ req(rv$kpis); fmt_pct(rv$kpis$mortgage30) })
  output$kpi_mortgage_chg <- renderUI({ HTML("") })

  output$kpi_housing <- renderText({ req(rv$kpis); fmt_num(rv$kpis$housing_starts, 0, "K") })
  output$kpi_housing_chg <- renderUI({ HTML("") })

  output$kpi_consump <- renderText({ req(rv$kpis); fmt_num(rv$kpis$cons_sent) })
  output$kpi_consump_chg <- renderUI({
    req(rv$kpis)
    v <- rv$kpis$cons_sent %||% NA
    HTML(paste0('<span class="metric-change ', if(!is.na(v) && v > 80) "positive" else "negative", '">',
                if(!is.na(v) && v > 80) "Optimistic" else "Pessimistic", '</span>'))
  })

  output$kpi_jolts   <- renderText({ req(rv$kpis); fmt_num(rv$kpis$job_openings, 0, "K") })
  output$kpi_jolts_chg <- renderUI({ HTML("") })

  output$last_updated <- renderText({
    if (is.null(rv$last_updated)) return("—")
  ...
  output$ov_unemp <- renderPlotly({
    ...
    plot_ly(df, x = ~date, y = ~value, type = "scatter", mode = "lines",
            line = list(color = "#00b4d8", width = 2.5), name = "Unemployment Rate",
  ...
  output$ov_cpi <- renderPlotly({
    ...
    if (!is.null(rv$fred_data$CPIAUCSL) && nrow(rv$fred_data$CPIAUCSL) > 12) {
      cpi_yoy <- rv$fred_data$CPIAUCSL %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
      plots[["CPI YoY"]] <- list(x=cpi_yoy$date, y=cpi_yoy$yoy, color="#e94560")
    }
    if (!is.null(rv$fred_data$CPILFESL) && nrow(rv$fred_data$CPILFESL) > 12) {
      core_yoy <- rv$fred_data$CPILFESL %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
      plots[["Core CPI YoY"]] <- list(x=core_yoy$date, y=core_yoy$yoy, color="#f4a261")
    }
    p <- plot_ly()
    for (nm in names(plots)) {
      p <- p %>% add_lines(x=plots[[nm]]$x, y=plots[[nm]]$y,
                            line=list(color=plots[[nm]]$color, width=2),
                            name=nm, hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>")
    }
    p %>% add_segments(x=min(do.call(c, lapply(plots, `[[`, "x"))),
                        xend=max(do.call(c, lapply(plots, `[[`, "x"))),
                        y=2, yend=2,
                        line=list(color="#ffffff44", width=1, dash="dot"),
                        name="2% Target", showlegend=TRUE, hoverinfo="none") %>%
      dark_theme(y="%")
  })
  ...
  output$ov_rates <- renderPlotly({
    ...
    for (nm in names(series_map)) {
      ...
      if (!is.null(df) && nrow(df) > 0) {
        ...
        p <- p %>% add_lines(x=df$date, y=df$value,
  ...
  output$ov_market <- renderPlotly({
    ...
    if (length(avail) == 0) return(empty_plot("Market data unavailable"))
    cutoff <- Sys.Date() - 365
    df <- rv$mkt_data %>% filter(symbol %in% avail, date >= cutoff)
    df <- df %>% group_by(symbol) %>%
      mutate(idx = close / first(close) * 100) %>% ungroup()
    cols <- c(SPY="#00b4d8", QQQ="#7c5cbf", GLD="#f4a261", TLT="#2dce89")
    p <- plot_ly()
    for (t in avail) {
      sub <- df %>% filter(symbol == t)
      p <- p %>% add_lines(data=sub, x=~date, y=~idx,
                             line=list(color=cols[t] %||% "#aaaaaa", width=2),
                             name=TICKER_LABELS[t] %||% t,
                             hovertemplate="%{x|%b %d}: %{y:.1f}<extra></extra>")
    }
    p %>% add_segments(x=min(df$date), xend=max(df$date), y=100, yend=100,
  ...
  output$lab_unemp <- renderPlotly({
    ...
    plot_ly(df, x=~date, y=~value, type="scatter", mode="lines",
            line=list(color="#00b4d8", width=2.5), fill="tozeroy",
            fillcolor="rgba(0,180,216,0.08)", name="Unemployment Rate",
            hovertemplate="%{x|%b %Y}: %{y:.1f}%<extra></extra>") %>%
      dark_theme(y="%")
  })
  ...
  output$lab_payrolls <- renderPlotly({
    ...
    df <- rv$fred_data$PAYEMS %>% tail(120) %>%
      mutate(
        mom = c(NA, diff(value)),
    ...
    plot_ly(df, x = ~date, y = ~mom, type = "bar",
  ...
  output$lab_jolts <- renderPlotly({
    ...
    if (!is.null(rv$fred_data$JTSJOL)) {
      ...
      p <- p %>% add_lines(data=df, x=~date, y=~value,
                             line=list(color="#00b4d8", width=2), name="Job Openings",
                             hovertemplate="%{x|%b %Y}: %{y:,.0f}K<extra></extra>")
    }
    if (!is.null(rv$fred_data$JTSQUL)) {
      df <- rv$fred_data$JTSQUL %>% tail(84)
      p <- p %>% add_lines(data=df, x=~date, y=~value,
                             line=list(color="#f4a261", width=2), name="Quits",
                             hovertemplate="%{x|%b %Y}: %{y:,.0f}K<extra></extra>")
    }
  ...
  output$lab_wages <- renderPlotly({
    ...
    df <- rv$fred_data$CES0500000003 %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
    plot_ly(df, x=~date, y=~yoy, type="scatter", mode="lines",
            line=list(color="#f4a261", width=2.5), fill="tozeroy",
            fillcolor="rgba(244,162,97,0.1)", name="Wage Growth YoY",
  ...
  output$inf_cpi <- renderPlotly({
    req(rv$fred_data)
    p <- plot_ly()
    for (pair in list(list("CPIAUCSL","CPI YoY","#e94560"),
                      list("CPILFESL","Core CPI YoY","#f4a261"))) {
      df <- rv$fred_data[[pair[[1]]]]
      if (!is.null(df) && nrow(df) > 12) {
        d <- df %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
        p <- p %>% add_lines(data=d, x=~date, y=~yoy,
                               line=list(color=pair[[3]], width=2.5), name=pair[[2]],
                               hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>")
      }
    }
    p %>% add_segments(x=as.Date("2018-01-01"), xend=Sys.Date(), y=2, yend=2,
                        line=list(color="#ffffff44", width=1, dash="dot"),
                        showlegend=FALSE, hoverinfo="none") %>%
      dark_theme(y="YoY %")
  })
  ...
  output$inf_pce <- renderPlotly({
    ...
    for (pair in list(list("PCEPI",   "PCE YoY",      "#7c5cbf"),
  ...
  output$inf_ppi <- renderPlotly({
    req(rv$fred_data)
    p <- plot_ly()
    for (pair in list(list("PPIACO","PPI YoY","#e94560"),
  ...
  output$inf_compare <- renderPlotly({
    req(rv$infl_panel)
    if (nrow(rv$infl_panel) == 0) return(empty_plot("BLS/FRED data unavailable"))
    cols <- c("CPI-U YoY (BLS)"="#00b4d8","CPI-U YoY (FRED)"="#e94560","Core CPI YoY"="#f4a261","PCE YoY"="#7c5cbf")
    p <- plot_ly()
    for (s in unique(rv$infl_panel$series)) {
      d <- rv$infl_panel %>% filter(series==s) %>% tail(84)
      p <- p %>% add_lines(data=d, x=~date, y=~value,
                             line=list(color=cols[s] %||% "#aaaaaa", width=2), name=s,
                             hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>")
    }
    p %>% dark_theme(y="YoY %")
  })
  ...
  output$hous_starts <- renderPlotly({
    ...
    if (!is.null(rv$fred_data$HOUST)) {
      ...
      p <- p %>% add_lines(data=df, x=~date, y=~value,
                             line=list(color="#2dce89", width=2.5), name="Housing Starts",
                             hovertemplate="%{x|%b %Y}: %{y:,.0f}K<extra></extra>")
    }
    if (!is.null(rv$fred_data$PERMIT)) {
      df <- rv$fred_data$PERMIT %>% tail(120)
      p <- p %>% add_lines(data=df, x=~date, y=~value,
                             line=list(color="#00b4d8", width=2, dash="dash"), name="Building Permits",
                             hovertemplate="%{x|%b %Y}: %{y:,.0f}K<extra></extra>")
    }
  ...
  output$hous_mortgage <- renderPlotly({
    ...
    plot_ly(df, x=~date, y=~value, type="scatter", mode="lines",
            ...
            fillcolor="rgba(233,69,96,0.08)", name="30-Yr Fixed",
            hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>") %>%
      dark_theme(y="%")
  })
  ...
  output$hous_nexus <- renderPlotly({
    ...
    mort  <- rv$fred_data$MORTGAGE30US %>% tail(120) %>% select(date, mort=value)
    start <- rv$fred_data$HOUST %>% tail(120) %>% select(date, starts=value)
    df    <- inner_join(mort, start, by="date") %>% filter(!is.na(mort), !is.na(starts))

    plot_ly(df) %>%
      add_lines(x=~date, y=~mort, name="Mortgage Rate (%)",
                line=list(color="#e94560", width=2.5),
                hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>") %>%
      add_lines(x=~date, y=~starts, name="Housing Starts (K)",
                line=list(color="#2dce89", width=2.5),
                yaxis="y2", hovertemplate="%{x|%b %Y}: %{y:,.0f}K<extra></extra>") %>%
      layout(
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=list(color="#cccccc"),
        yaxis=list(title="Mortgage Rate (%)", gridcolor="#2a3042", color="#e94560"),
        yaxis2=list(title="Housing Starts (K)", overlaying="y", side="right",
                     gridcolor="transparent", color="#2dce89"),
  ...
  output$con_retail <- renderPlotly({
    ...
    df <- rv$fred_data$RSAFS %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
    plot_ly(df, x=~date, y=~yoy, type="bar",
            marker=list(color=if_else(df$yoy >= 0, "#00b4d8", "#e94560")),
            name="Retail Sales YoY", hovertemplate="%{x|%b %Y}: %{y:.1f}%<extra></extra>") %>%
      dark_theme(y="YoY %")
  })
  ...
  output$con_sent <- renderPlotly({
    ...
    plot_ly(df, x=~date, y=~value, type="scatter", mode="lines",
            line=list(color="#7c5cbf", width=2.5), fill="tozeroy",
            fillcolor="rgba(124,92,191,0.08)", name="Consumer Sentiment",
            hovertemplate="%{x|%b %Y}: %{y:.1f}<extra></extra>") %>%
      dark_theme(y="Index")
  })
  ...
  output$con_indpro <- renderPlotly({
    ...
    df <- rv$fred_data$INDPRO %>% yoy_change() %>% filter(!is.na(yoy)) %>% tail(84)
    plot_ly(df, x=~date, y=~yoy, type="scatter", mode="lines",
            line=list(color="#2dce89", width=2.5), fill="tozeroy",
            fillcolor="rgba(45,206,137,0.08)", name="Industrial Production YoY",
            hovertemplate="%{x|%b %Y}: %{y:.2f}%<extra></extra>") %>%
      dark_theme(y="YoY %")
  })
  ...
  output$tbl_mkt <- renderDT({
    ...
    if (is.null(tbl)) return(datatable(data.frame()))
    datatable(tbl, options=list(pageLength=15, dom="ft"), rownames=FALSE,
              class="table-dark compact stripe") %>%
      ...
      formatStyle("YTD %",color=styleInterval(0, c("#e94560","#2dce89"))) %>%
      formatStyle(columns=names(tbl), backgroundColor="#1e2640", color="#d0d0d0")
  })
  ...
  output$mkt_price_chart <- renderPlotly({
    ...
    df <- rv$mkt_data %>% filter(symbol %in% tickers, date >= cutoff) %>%
      group_by(symbol) %>%
      mutate(idx = close / first(close) * 100) %>% ungroup()
    if (nrow(df) == 0) return(empty_plot("No data for selected tickers"))
    pal <- c("#00b4d8","#e94560","#2dce89","#f4a261","#7c5cbf","#a8dadc","#ff6b6b","#ffd93d")
    p   <- plot_ly()
  ...
  output$mkt_sector_heat <- renderPlotly({
    ...
    if (is.null(df) || nrow(df)==0) return(empty_plot("Sector data unavailable"))
    plot_ly(df, x=~name, y=~ret_1m, type="bar",
      ...
      layout(
        ...
        yaxis=list(title="1-Month Return %", gridcolor="#2a3042", color="#9aa3b2",
                   zeroline=TRUE, zerolinecolor="#ffffff55"),
  ...
  output$mkt_commodities <- renderPlotly({
    ...
    if (length(avail)==0) return(empty_plot("Commodity ETF data unavailable"))
    df  <- rv$mkt_data %>% filter(symbol %in% avail, date >= Sys.Date()-365) %>%
      group_by(symbol) %>% mutate(idx=close/first(close)*100) %>% ungroup()
  ...
  output$mkt_yield_curve <- renderPlotly({
    ...
    vals <- vapply(tenors, function(t) {
      df <- rv$fred_data[[t]]
      if (!is.null(df) && nrow(df)>0) tail(df$value,1) else NA_real_
    }, numeric(1))

    df_yc <- tibble(tenor=factor(labels,levels=labels), yield=vals) %>% filter(!is.na(yield))
    if (nrow(df_yc) < 3) return(empty_plot("Yield curve data incomplete (add DGS tenor series to FRED_CFG)"))

    plot_ly(df_yc, x=~tenor, y=~yield, type="scatter", mode="lines+markers",
            line=list(color="#2dce89", width=2.5),
  ...
  output$mkt_corr <- renderPlotly({
    req(rv$fred_data)
    sids <- intersect(
      c("UNRATE","CPIAUCSL","FEDFUNDS","DGS10","DCOILWTICO","VIXCLS","RSAFS","MORTGAGE30US"),
      names(rv$fred_data)
    )
    sids <- sids[!sapply(sids, function(s) is.null(rv$fred_data[[s]]))]
    if (length(sids) < 3) return(empty_plot("Insufficient data for correlation"))

    cor_mat <- fred_correlation_matrix(rv$fred_data, sids)
    labs    <- sapply(sids, function(s) FRED_CFG[[s]]$label %||% s)

    plot_ly(x=labs, y=labs, z=cor_mat, type="heatmap",
  ...
  observeEvent(input$tabs, {
    if (input$tabs == "metro") {
      if (is.null(rv$census_data)) {
        withProgress(message = "Loading state ACS data from Census API…", value = 0, {
  ...
  output$metro_map <- renderLeaflet({
    ...
    map_out <- tryCatch(
      ...
      error = function(e) { message("[Map error] ", e$message); NULL }
    )
    if (is.null(map_out)) {
      leaflet::leaflet() %>%
        leaflet::addProviderTiles("CartoDB.DarkMatter") %>%
  ...
  output$tbl_metro <- renderDT({
    ...
    datatable(df,
              ...
              rownames = FALSE, class = "table-dark compact stripe") %>%
      formatStyle("Unemp Rate %",
                  color = styleInterval(c(4, 6), c("#2dce89","#f4a261","#e94560"))) %>%
      formatStyle(columns = seq_len(7),
                  backgroundColor = "#1e2640", color = "#d0d0d0")
  })

  # Metro CBSA rankings table
  output$tbl_metro_cbsa <- renderDT({
    if (is.null(rv$metro_data) || nrow(rv$metro_data) == 0) {
      return(datatable(data.frame(Message = "Metro data not yet loaded or unavailable."),
                       rownames = FALSE))
    }
    df <- rv$metro_data %>%
      transmute(
  ...
  output$metro_bubble_map <- renderLeaflet({
    ...
    map_out <- tryCatch(
      ...
      error = function(e) { message("[Metro bubble error] ", e$message); NULL }
    )
  ...
  observeEvent(input$fcst_horizon, {
    req(rv$fred_data)
    withProgress(message="Re-running forecasts…", value=0, {
      rv$forecasts <- run_all_forecasts(rv$fred_data, horizon_months=input$fcst_horizon)
      incProgress(1)
    })
  }, ignoreInit=TRUE)
  ...
  observeEvent(input$llm_provider, {
    ...
    updateSelectInput(session, "llm_model",
                      choices  = models,
                      selected = models[1])
  }, ignoreNULL=FALSE)
  ...
  output$llm_status_badge <- renderUI({
    ...
    if (is.null(prov)) {
      ...
      if (key_ok) span(class="status-badge badge-green", icon("check"), " Ready")
      else span(class="status-badge badge-red", icon("key"), " No Key")
    }
  })
  ...
  output$provider_badge_sidebar <- renderUI({
    ...
    if (is.null(prov)) {
      div(style="margin-top:8px;",
          span(style="color:#e94560;font-size:11px;", icon("exclamation-triangle"), " LLM not configured"))
    } else {
      div(style="margin-top:8px;",
          span(class="provider-badge", icon("robot"), " ", LLM_PROVIDERS[[prov]]$display_name))
    }
  })
  ...
  output$llm_output <- renderUI({
    if (is.null(rv$llm_text)) {
      div(style="color:#6b7585;font-size:13px;padding:20px;text-align:center;",
          icon("robot", style="font-size:32px;color:#2a3042;margin-bottom:12px;display:block;"),
          "Click \"Generate AI Insights\" to synthesise live economic data and headlines.", br(),
  ...
  output$chat_answer <- renderUI({
    if (is.null(rv$chat_text)) return(NULL)
    div(style="background:#1e2640;border-radius:8px;padding:14px;margin-top:10px;font-size:13px;line-height:1.7;",
