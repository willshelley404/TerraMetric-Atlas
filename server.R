...
server <- function(input, output, session) {
    ...
    tryCatch(
      {
        ...
        rv$fred_data <- patch_wti_realtime(rv$fred_data) # ← bridge the FRED lag
    ...
    rows <- map_dfr(
      ...
      function(sid) {
        ...
        tibble(
          ...
          Context = if (sid == "CPIAUCSL") {
            "Headline; Fed target 2%"
          } else if (sid == "CPILFESL") {
            "Ex food & energy"
          } else if (sid == "PCEPI") {
            "PCE headline"
          } else if (sid == "PCEPILFE") {
            "Fed preferred gauge"
          } else if (sid == "PPIACO") {
            "Leads CPI by ~2 months"
          } else {
            ""
          }
        )
    ...
    datatable(
      ...
      formatStyle(
        ...
        color = styleEqual(
          rows$`Δ Prev`,
          ifelse(
            as.numeric(gsub(" pp", "", rows$`Δ Prev`)) >= 0,
            "#e94560",
            "#2dce89"
          )
        )
  ...
  observeEvent(input$btn_metro_ai, {
    ...
    withProgress(
      message = paste("Analysing", input$metro_ai_select, "…"),
      value = 0.2,
      {
        ...
        summary_obj <- tryCatch(
          ...
          error = function(e) {
            message("[Metro AI] build_metro_summary: ", e$message)
            NULL
          }
        )
  ...
  output$metro_ai_output <- renderUI({
    if (is.null(rv_metro_ai$text)) {
      div(
        ...
        icon(
          "robot",
          style = "font-size:28px;color:#2a3042;display:block;margin-bottom:10px;"
        ),
        "Select a metro and click Generate to get a structured economic assessment.",
        tags$br(),
        div(
          style = "color:#555;font-size:11px;margin-top:8px;",
  ...
  observeEvent(input$cfg_apply, {
    ...
    w$hide()
    showNotification(
      "Settings applied. Data refreshed.",
      type = "message",
      duration = 4
    )
  })
  ...
  output$growth_outlook_panel <- renderUI({
    ...
    outlook <- tryCatch(
      ...
      error = function(e) {
        message("[Growth outlook] ", e$message)
        NULL
      }
    )
  ...
  .tab_synopsis <- function(fn) {
    renderUI({
      if (is.null(rv$fred_data) || is.null(rv$kpis)) {
        return(NULL)
      }
      tryCatch(fn(rv$fred_data, rv$kpis), error = function(e) {
        message("[Tab synopsis] ", e$message)
        NULL
      })
    })
  }
