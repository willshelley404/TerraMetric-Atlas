...
FRED_CFG <- list(
  ...
  GOLDPMGBD228NLBM = list(
    label = "Gold Price (London PM)",
    unit = "$/oz",
    tab = "markets"
  ),
...
LLM_PROVIDERS <- list(
  groq = list(
    ...
    models = c("llama-3.3-70b-versatile", "mixtral-8x7b-32768", "gemma2-9b-it"),
    display_name = "Groq"
  ),
  openrouter = list(
...
detect_llm_provider <- function() {
  ...
  if (nchar(forced) > 0 && forced %in% names(LLM_PROVIDERS)) {
    return(forced)
  }
  for (p in names(LLM_PROVIDERS)) {
    if (nchar(Sys.getenv(LLM_PROVIDERS[[p]]$key_env)) > 0) return(p)
  }
  NULL
}
  ...
  p %>%
    layout(
      title = if (!is.null(title)) {
        list(text = title, font = list(color = "#e0e0e0", size = 14))
      } else {
        NULL
      },
...
pct_change <- function(v) {
  n <- length(v)
  if (n < 2 || any(is.na(v[c(n - 1, n)]))) {
    return(NA_real_)
  }
  (v[n] / v[n - 1] - 1) * 100
}
yoy_change <- function(df, lag_n = 12) {
  df %>% arrange(date) %>% mutate(yoy = (value / lag(value, lag_n) - 1) * 100)
}
